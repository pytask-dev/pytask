...
class Task(PTaskWithPath):
    ...
    def __rich_console__(
        ...
        table = Table(
            "", "", title="Information", show_header=False, expand=True, box=box.ROUNDED
        )
        table.add_row("Name", self.name)
        table.add_row("Path", self.path.as_posix())
        table.add_row("# Dependencies", str(len(tree_leaves(self.depends_on))))
        table.add_row("# Products", str(len(tree_leaves(self.produces))))

        yield table
        yield Panel(inspect.getdoc(self.function), title="Docstring")
        yield Panel(
            Syntax(inspect.getsource(self.function), lexer="python"),
            title="Source Code",
        )
