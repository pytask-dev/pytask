...
As a solution, pytask offers a \[`pytask.DataCatalog`\][] which is a purely optional
...
Let us focus on the previous example and see how the \[`pytask.DataCatalog`\][] helps
us.
...
`"data"`, the data catalog will automatically create a \[`pytask.PickleNode`\][]. The
...
catalog. To add them, call the \[`pytask.DataCatalog.add`\][] method and supply a name
and a path.
...
Simply import the data catalog, select a node and call the \[`pytask.PNode.load`\][]
