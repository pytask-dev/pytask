...
can be represented as plain Python objects with `pytask.PythonNode` or as paths where
every `pathlib.Path` is converted to a `pytask.PathNode`.
...
follow at least the protocol `pytask.PNode` or, even better, `pytask.PPathNode` if it is
based on a path. The common node for paths, `pytask.PathNode`, follows the protocol
`pytask.PPathNode`.

## `PickleNode`

Since our `pytask.PickleNode` will only vary slightly from `pytask.PathNode`, we use it
as a template, and with some minor modifications, we arrive at the following class.
