...
[`pytask.PNode`](../reference_guides/api/nodes_and_tasks.md#pytask.PNode) to the task.
If you passed a node as the default argument, type checkers like mypy would expect the
node to enter the task, but the value injected into the task depends on the nodes
[`pytask.PNode.load`](../reference_guides/api/nodes_and_tasks.md#pytask.PNode.load)
method. For a
...
[`@task`](../reference_guides/api/nodes_and_tasks.md#pytask.task) decorator.
