# How-to Guides

This section contains two collections of documents.

## How-to Guides

The first collection of how-to guides provides detailed explanations on accomplishing
specific tasks with pytask.

- [Migrating From Scripts To Pytask](migrating_from_scripts_to_pytask.md)
- [Interfaces For Dependencies Products](interfaces_for_dependencies_products.md)
- [Portability](portability.md)
- [Remote Files](remote_files.md)
- [Functional Interface](functional_interface.md)
- [Capture Warnings](capture_warnings.md)
- [How To Influence Build Order](how_to_influence_build_order.md)
- [Hashing Inputs Of Tasks](hashing_inputs_of_tasks.md)
- [Using Task Returns](using_task_returns.md)
- [Provisional Nodes And Task Generators](provisional_nodes_and_task_generators.md)
- [Writing Custom Nodes](writing_custom_nodes.md)
- [Extending Pytask](extending_pytask.md)
- [The Data Catalog](the_data_catalog.md)
- [Using Workspaces](using_workspaces.md)

## Best Practice Guides

The second collection comprises best practice guides for pytask. The guides combine
experience with pytask and build systems in general, research projects, and software
engineering to provide practical and easily understandable instructions.

Contributions - additions, comments, experiences, and requests for clarification - are
highly appreciated. File either an [issue](https://github.com/pytask-dev/pytask/issues)
or start a [discussion](https://github.com/pytask-dev/pytask/discussions).

- [Bp Structure Of A Research Project](bp_structure_of_a_research_project.md)
- [Bp Structure Of Task Files](bp_structure_of_task_files.md)
- [Bp Templates And Projects](bp_templates_and_projects.md)
- [Bp Complex Task Repetitions](bp_complex_task_repetitions.md)
